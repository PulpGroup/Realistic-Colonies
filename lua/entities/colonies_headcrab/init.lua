...
function ENT:Initialize()
    ...
    self.npc:SetNWString("rc_class", "headcrab")
...
function ENT:Think()
    ...
    self.hunger = self.hunger + GetConVarNumber("rc_headcrab_hunger") * GetConVarNumber("rc_time") *
    ...
    if (IsValid(self.npc) ~= true) then
        ...
        if GetConVarNumber("rc_spreadthelove") == 0 and self.hunger > GetConVarNumber("rc_headcrab_mhunger") *
        ...
        if self.hunger > GetConVarNumber("rc_headcrab_mhunger") * GetConVarNumber("rc_hungry") / 100 then
            ...
            -- Laying egg time
            if self.age > self.nextegg and headcrabCount() <= GetConVarNumber("rc_headcrab_max") and self.hunger <=
                self.mhunger / 2 then
        ...
        elseif self.age <= GetConVarNumber("rc_headcrab_maturetime") then
            self.scale = 0.5 +
