...
function ENT:Think()
    ...
    self.hunger = self.hunger + GetConVarNumber("rc_zombie_hunger") * GetConVarNumber("rc_time") *
    ...
    if (IsValid(self.npc) ~= true) then
        ...
        if GetConVarNumber("rc_spreadthelove") == 0 and self.hunger > GetConVarNumber("rc_zombie_mhunger") *
        ...
        if self.hunger > GetConVarNumber("rc_zombie_mhunger") * GetConVarNumber("rc_hungry") / 100 then
            ...
            -- Laying egg time
            if self.age > self.nextegg and zombieCount() <= GetConVarNumber("rc_zombie_max") and self.hunger <=
                self.mhunger / 2 then
        ...
        if self.age <= GetConVarNumber("rc_zombie_maturetime") * 0.25 then
            self.scale = 0.2 + (2 * self.age / GetConVarNumber("rc_zombie_maturetime")) * (0.8)
