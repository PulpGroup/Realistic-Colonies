	...
	function ENT:Think()
		...
		if( !IsValid(self.npc) ) then
			...
			local meat = ents.Create("colonies_hmeat")
			meat:SetPos(self:GetPos()+Vector(5,0,10010))
			meat:SetModelScale(self.scale,0);
			meat:Spawn()
			meat:SetOwner(self.Owner)
			local meat = ents.Create("colonies_hmeat")
			meat:SetPos(self:GetPos()+Vector(0,5,10010))
			meat:SetModelScale(self.scale,0);
			meat:Spawn()
			self:Remove()
		else
			...
			if self.hunger > 40 then
				...
				if self.hunger >= GetConVarNumber("rc_zombie_mhunger") then
					...
					if(self.npc:Health() <= 0) then
						...
						meat:SetOwner(self.Owner)
						local meat = ents.Create("colonies_hmeat")
						meat:SetPos(self:GetPos()+Vector(0,2,10010))
						meat:SetModelScale(self.scale,0);
						meat:Spawn()
						meat:SetOwner(self.Owner)
						local meat = ents.Create("colonies_hmeat")
						meat:SetPos(self:GetPos()+Vector(2,0,10010))
						meat:SetModelScale(self.scale,0);
						meat:Spawn()
						self:Remove()
					end
